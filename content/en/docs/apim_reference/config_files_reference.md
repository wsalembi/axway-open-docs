...
| `com.axway.apimanager.csrf` | boolean | `true` | If you are using the API Manager Management APIs, Client Application Registry APIs, and API Gateway APIs you might need to disable the CSRF token check implemented in v7.5.3 SP9 and later. To disable this check, set the value to `false`. |
...
| `com.vordel.apimanager.uri.path.trailingSlash.preserve` | boolean | `false` |  When a back-end API is created from a Swagger definition file that contains trailing slashes in the path, API Manager removes the trailing slashes from the paths. Furthermore, when a request comes in with a trailing slash, API Gateway returns HTTP error `403 bad request` because it does not match the requested path. To preserve the trailing forward slashes, set to `true`. |
| `com.vordel.dwe.auto204response`  | boolean | `true` | API Gateway rewrites the `HTTP 200 OK` status code by default to `HTTP 204 No Content` if the message payload is empty. When set to `false` you can disable this behavior, e.g. if you require that `HEAD` requests keep the  exact same status code as `GET`.  |
| `dont.expect.100.continue` | boolean | `false` | Using an `HTTP 1.1` remote host API gateway always sends an `Expect: 100-Continue` HTTP header to the backend server. This behavior can be be disabled by setting the value to `true` |
